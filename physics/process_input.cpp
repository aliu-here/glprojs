...
class callback_funcs 
{
	public:
		...
		float scrollSens;
		float mouseSens;
        bool freeze_phys = false;
        float lastPhysToggle;
	...
	void process_kb_input(GLFWwindow *window, float deltaTime, float currentTime)
	{
		...
		if (glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS && trueUp)
			cameraPos -= up * cameraSpeed;
        if (glfwGetKey(window, GLFW_KEY_P) == GLFW_PRESS && currentTime - lastPhysToggle >= 0.5f) {
            freeze_phys = !freeze_phys;
            lastPhysToggle = currentTime;
        }
	}
