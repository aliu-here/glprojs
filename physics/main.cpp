...
//	std::string path = getExecPath();
    ...
    std::cout << model.model.size() << '\n';
    glm::vec3 lightSourceLoc = model.model[0].bounding_box.max;
...
//    glEnable(GL_CULL_FACE);
	...
	while(!glfwWindowShouldClose(window))
	{
        ...
        for (physics_obj *obj : objs) {
            glm::mat4 model = glm::mat4(1.0f); // make sure to initialize matrix to identity matrix first
            model = glm::translate(model, obj->pos);
		    mainShader.setMat4("model", model);
		    mainShader.setVec3("objectColor", obj->model[0].used_mtl.ambient);
        }
...
//		camera.cameraPos = lightSourceLoc;
        ...
        if (!camera.freeze_phys)
            update_objs(objs, deltaTime);
