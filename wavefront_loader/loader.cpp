...
//              std::cout << split_line[0] << '\n';
    ...
    std::vector<triangle> generate_triangles(const std::vector<std::string>& data, int line_num, const std::vector<std::vector<float>>& vert_data, const std::vector<std::vector<float>>& uv_coord_data, const std::vector<std::vector<float>>& normal_data, int vertex_offset, int uv_offset, int normal_offset, mesh& curr_group)
    {
...
//                std::cout << "before converting split data to int\n";
                ...
                for (int i=0; i<8; i++) { temp.points[curr_point].point_data[i] = 0.0f; }
...
//                    std::cout << join(data, " ") << '\n';
                    for (int i=0; i<3; i++) {
                        ...
                        curr_group.bounding_box.coords[1][i] = std::min(curr_group.bounding_box.coords[1][i], vert_data[locs[0]][i]);
                    }
                }
                switch(locs.size())
                {
...
//            std::cout << line_type << ' ' << str_arg1 << ' ' << str_arg2 << ' ' << str_arg3 << '\n';
            ...
            if (line_type == "v") { //vertex
                ...
                std::vector<triangle> temp_triangles = generate_triangles(temp, line_num, vert_data, uv_coord_data, normal_data, vertex_offset, uv_offset, normal_offset, curr_group);
